	...
	async function handleSubmit(e: SubmitEvent) {
		...
		try {
			...
			if (json.success) {
				...
				errorMsg = json.error || 'Login failed.';
			}
		} catch (err: any) {
...
<svelte:head>
	<title>Sign In - AeroReliability</title>
</svelte:head>
	...
	<div class="max-w-md w-full space-y-8 hud-card p-8 border-hud-cyan/20">
		<div class="text-center">
			<div class="inline-flex w-12 h-12 rounded border border-hud-cyan flex items-center justify-center bg-hud-cyan/10 mb-4">
				<svg class="w-6 h-6 text-hud-cyan" fill="none" viewBox="0 0 24 24" stroke="currentColor">
			...
			<p class="mt-2 text-sm text-gray-400">
				Access your saved reliability engineering projects.
			</p>
		</div>
		...
		<form class="mt-8 space-y-6" onsubmit={handleSubmit}>
			<div class="space-y-4">
				...
				<div>
					<label for="password" class="block text-xs font-mono text-gray-400 mb-1">SECRET_PASSKEY</label>
					<input 
						id="password" 
						type="password" 
						required 
			...
			<div class="text-center text-xs font-mono">
				<span class="text-gray-500">Need a new credentials file?</span>
				<a href="/register" class="text-hud-cyan hover:underline ml-1">Create Account</a>
			</div>
		</form>
	</div>
</div>
